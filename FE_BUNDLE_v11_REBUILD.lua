--[[
====================================================================
FE_BUNDLE v11 — FULL ARCHITECTURE REBUILD
====================================================================
Version: 11.0.0
Schema: 1
Architecture: single coherent application (NO V12/V13/V14 patches)
...
local SAVE_FILE = "FE_BUNDLE_V11.json"
...
local function resolveEmoteAnimationId(assetId)
    ...
    pcall(function()
        ...
        for _, obj in ipairs(objs or {}) do
            if obj:IsA("Animation") then found = normalizeId(obj.AnimationId)
            else
                for _, d in ipairs(obj:GetDescendants()) do
                    if d:IsA("Animation") then found = normalizeId(d.AnimationId); break end
                end
            end
    ...
    if App.Settings.CacheUGCIds then App.EmoteAnimCache[assetId] = found; saveData() end
    return found
end

local function stopEmote()
    if App.CurrentEmoteTrack then
        pcall(function() App.CurrentEmoteTrack:Stop(0.1); App.CurrentEmoteTrack:Destroy() end)
        App.CurrentEmoteTrack = nil
    end
end

local function stopReverse()
    if App.ReverseConn then
        pcall(function() App.ReverseConn:Disconnect() end)
        App.ReverseConn = nil
    end
end

local function applyTrackControls(track)
    if not track then return end
    pcall(function()
        track.Looped = App.Controller.Loop
        track:AdjustSpeed(App.Controller.Reverse and -math.abs(App.Controller.Speed) or App.Controller.Speed)
        if track.AdjustWeight then
            track:AdjustWeight(math.clamp(App.Controller.Intensity, 0.05, 2), 0.1)
        end
    end)
    stopReverse()
    if App.Controller.Reverse and App.Controller.Speed ~= 0 then
        -- Fallback reverse if AdjustSpeed negative not supported
        App.ReverseConn = RunService.Heartbeat:Connect(function(dt)
            if not track or not track.IsPlaying then stopReverse(); return end
            pcall(function()
                local len = track.Length
                if len and len > 0 then
                    local tp = track.TimePosition - math.abs(App.Controller.Speed) * dt
                    if tp < 0 then
                        if App.Controller.Loop then tp = len + tp
                        else track:Stop(); stopReverse(); return end
                    end
                    track.TimePosition = tp
                end
            end)
        end)
        connAdd("Controller", App.ReverseConn)
    end
end

local function playEmote(catalogOrAnimId, name)
    local realId = resolveEmoteAnimationId(catalogOrAnimId)
    if not realId or realId=="" then status("No animation ID", false); notify("FE_BUNDLE","Animation failed"); return end
    local animator, hum = getAnimator()
    if not animator then status("No Animator", false); return end
    stopEmote()
    local anim = Instance.new("Animation")
    anim.AnimationId = toAnimUrl(realId)
    local ok, track = pcall(function() return animator:LoadAnimation(anim) end)
    if not ok or not track then status("Load failed", false); notify("FE_BUNDLE","Animation failed"); return end
    App.CurrentEmoteTrack = track
    pcall(function()
        track.Priority = App.Settings.MoveWhileEmote and Enum.AnimationPriority.Action or Enum.AnimationPriority.Action4
        track.Looped = App.Settings.EmoteLoop
        track:Play(0.12, 1, App.Settings.EmoteSpeed)
    end)
    applyTrackControls(track)
    status("Playing: "..tostring(name or realId), true)
    -- refresh controller track list
    if refreshControllerTracks then refreshControllerTracks() end
end
...
local function showLoading(text)
    ...
    local card = new("Frame", {
        ...
        Size=UDim2.new(0,260,0,80), BackgroundColor3=Theme.Surface, BorderSizePixel=0, ZIndex=201
    })
    corner(card, 12); stroke(card, Theme.Accent, 1, 0.3)
    makeLabel(card, text or "Loading...", 14, Theme.Text, true).Position = UDim2.new(0,16,0,12)
    makeLabel(card, text or "Loading...", 14, Theme.Text, true).Size = UDim2.new(1,-32,0,28)
    local barBg = new("Frame", {Parent=card, Position=UDim2.new(0,16,0,48), Size=UDim2.new(1,-32,0,10), BackgroundColor3=Theme.Field, BorderSizePixel=0, ZIndex=202})
    corner(barBg, 6)
    local bar = new("Frame", {Parent=barBg, Size=UDim2.new(0.3,0,1,0), BackgroundColor3=Theme.Accent, BorderSizePixel=0, ZIndex=203})
    corner(bar, 6)
    task.spawn(function()
        while bar and bar.Parent do
            bar.Position = UDim2.new(-0.3,0,0,0)
            tween(bar, {Position=UDim2.new(1,0,0,0)}, 0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut)
            task.wait(0.85)
        end
    end)
    UI.Loading = dim
...
local function createAvatarPreview(parent, animId)
    clearPreviewTracks()
    -- Full-body viewport: no tight crop, camera pulled back
    local vp = new("ViewportFrame", {
        ...
        LightColor=Color3.fromRGB(255,255,255), LightDirection=Vector3.new(-0.4,-1,-0.6),
        ZIndex=220
    })
    corner(vp, 14)
    local world = Instance.new("WorldModel"); world.Parent = vp
    local char = LocalPlayer.Character
    if not char then return vp end
    ...
    if root then
        ...
        pcall(function()
            root.Anchored = true
            -- Stand centered, feet near bottom of view
    ...
    -- Wider FOV + further camera = full body visible (head to feet)
    cam.FieldOfView = 50
    -- Look at mid-torso from distance so legs + head fit
    cam.CFrame = CFrame.new(Vector3.new(0, 2.0, 7.5), Vector3.new(0, 1.2, 0))
...
local function showInfoModal(item, kind)
    ...
    local function openWith(realId, body)
        ...
        task.delay(0.05, function()
            ...
            if kind=="Emote" then
                makeBtn(actions, "PLAY", function() playEmote(id, name) end, Theme.Green, {bucket="Modal", size=UDim2.new(0,80,0,32)})
...
local function reflowFloating()
    ...
    if mode == "Autogrid" then
        ...
        for i, fb in ipairs(App.FloatingButtons) do
            ...
            corner(img, 8)
            local captured = fb
            connAdd("Floating", b.MouseButton1Click:Connect(function()
                playEmote(captured.animId or captured.catalogId or captured.id, captured.name)
            end))
            pcall(function() connAdd("Floating", b.Activated:Connect(function()
                playEmote(captured.animId or captured.catalogId or captured.id, captured.name)
            end)) end)
        end
    else -- Freeform
        ...
        for _, fb in ipairs(App.FloatingButtons) do
            ...
            corner(img, 8)
            local captured = fb
            local dragging, moved, inputRef, start, startPos
            local THRESH=12
            connAdd("Floating", b.InputBegan:Connect(function(input)
                if input.UserInputType==Enum.UserInputType.Touch or input.UserInputType==Enum.UserInputType.MouseButton1 then
                    dragging=true; moved=false; inputRef=input; start=input.Position; startPos=b.Position
                    local c; c=input.Changed:Connect(function()
                        if input.UserInputState==Enum.UserInputState.End then
                            dragging=false
                            if not moved then playEmote(captured.animId or captured.catalogId or captured.id, captured.name) end
            ...
            connAdd("Floating", UserInputService.InputChanged:Connect(function(input)
                if dragging and input==inputRef then
                    ...
                    if moved then
                        ...
                        captured.posX=b.Position.X.Scale; captured.posXO=b.Position.X.Offset
                        captured.posY=b.Position.Y.Scale; captured.posYO=b.Position.Y.Offset
                    end
                end
            end))
            connAdd("Floating", UserInputService.InputEnded:Connect(function(input)
                if input==inputRef and moved then saveData() end
            end))
        end
    end
end
...
local function rebuildQuickPanel()
    ...
    for _, qe in ipairs(App.QuickEntries) do
        ...
        connAdd("Page", b.MouseButton1Click:Connect(function()
            playEmote(captured.animId or captured.catalogId, captured.name)
        end))
    end
end
...
local function renderItemCard(parent, item, kind)
    ...
    card.Size = UDim2.new(1, 0, 0, isCompact() and 110 or 120)
    ...
    makeBtn(actions, kind=="Emote" and "PLAY" or (App.ChoosingState and "SET" or "APPLY"), function()
        if capturedKind=="Emote" then playEmote(tostring(capturedItem.id or capturedItem.Id), tostring(capturedItem.name or ""))
    ...
    end, kind=="Emote" and Theme.Green or Theme.Accent, {size=UDim2.new(0,78,0,28)})
...
local function renderEmotesPage()
    ...
    makeBtn(searchRow, "SEARCH", function()
        if App.Settings.SourceFilter=="Favorites" then navigate("Emotes"); return end
        showLoading("Searching...")
        task.spawn(function()
            local ok, count = searchCatalog("Emote", searchBox.Text, false)
            hideLoading()
    ...
    if #list==0 then
        ...
        if App.NextEmoteCursor and App.Settings.SourceFilter~="Favorites" then
            makeBtn(scroll, "LOAD MORE", function()
                showLoading("Loading...")
                task.spawn(function()
                    searchCatalog("Emote", App.LastEmoteKeyword, true)
                    hideLoading(); navigate("Emotes")
                end)
            end, Theme.Card, {size=UDim2.new(1,-8,0,34)})
        end
    end
end
...
local function renderBundlesPage()
    ...
    makeBtn(searchRow, "SEARCH", function()
        showLoading("Searching...")
        task.spawn(function()
            local ok, count = searchCatalog("Bundle", searchBox.Text, false)
            hideLoading()
    ...
    if #list==0 then makeLabel(scroll, "No bundles. Search first.", 14, Theme.LightMuted)
    else
        ...
        if App.NextBundleCursor then
            makeBtn(scroll, "LOAD MORE", function()
                showLoading("Loading...")
                task.spawn(function()
                    searchCatalog("Bundle", App.LastBundleKeyword, true)
                    hideLoading(); navigate("Bundles")
                end)
            end, Theme.Card, {size=UDim2.new(1,-8,0,34)})
        end
    end
...
local function renderSettingsPage()
    ...
    for _, n in ipairs({0.5,1,1.5,2}) do
        makeBtn(srow, tostring(n).."x", function()
            App.Settings.EmoteSpeed=n
            if App.CurrentEmoteTrack then pcall(function() App.CurrentEmoteTrack:AdjustSpeed(n) end) end
...
local function createGui()
    ...
    print("[FE11] parent=", how)
    if not pg then return false end

    pcall(function()
        for _, n in ipairs({"FE_BUNDLE_V11","FE_BUNDLE_V10_FIXED","FE_BUNDLE_V10","TEST_GUI_ONLY","FE_BUNDLE_MARKER"}) do
            local o = pg:FindFirstChild(n); if o then o:Destroy() end
        end
    end)

    local root = Instance.new("ScreenGui")
    root.Name = "FE_BUNDLE_V11"
    ...
    tag.Parent = root
    tag.Position = UDim2.new(0, 6, 0.38, 60)
    ...
    connAdd("Global", UserInputService.InputChanged:Connect(function(input)
        if iconDragging and input==iconInput then
            ...
            if iconMoved then
                ...
                icon.Position=np
                tag.Position=UDim2.new(np.X.Scale, np.X.Offset-8, np.Y.Scale, np.Y.Offset+60)
            end
        end
    end))

    applyLayout()
    print("[FE11] createGui OK")
    return true
end
...
-- BOOT
---------------------------------------------------------------------
print("[FE11] loading data...")
pcall(loadData)

local ok, err = pcall(createGui)
if not ok then
    print("[FE11] createGui ERROR:", err)
    notify("FE_BUNDLE", "Boot error")
elseif err == false then
    print("[FE11] createGui returned false")
else
    print("[FE11] SUCCESS")
