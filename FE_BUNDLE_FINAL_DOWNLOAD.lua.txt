...
local function makeButton(parent, text, pos, size, callback, color)
    ...
    local function pressAnim()
        ...
        corner(dot, 99)
        tween(dot, {Size = UDim2.new(1.8, 0, 1.8, 0), BackgroundTransparency = 1}, 0.25)
        task.delay(0.28, function() if dot then dot:Destroy() end end)
    end
    ...
    add(PageConnections, b.MouseButton1Down:Connect(pressAnim))
    add(PageConnections, b.MouseButton1Up:Connect(releaseAnim))
    add(PageConnections, b.MouseButton1Click:Connect(fire))
    add(PageConnections, b.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            pressAnim()
        end
    end))
    add(PageConnections, b.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            releaseAnim()
            fire()
        end
    end))
    pcall(function() add(PageConnections, b.Activated:Connect(fire)) end)
    return b
end
...
local function setPage(page)
    ...
    if Body then
        ...
        tween(Body, {BackgroundTransparency = 1}, 0.01)
        task.delay(0.03, function()
            if Body then tween(Body, {Position = UDim2.new(0, 12, 0, 66)}, 0.12) end
        end)
    end
end
...
local function createGui()
    ...
    add(Connections, IconButton.MouseButton1Click:Connect(toggleMain))
    add(Connections, IconButton.MouseButton1Down:Connect(toggleMain))
    pcall(function() add(Connections, IconButton.Activated:Connect(toggleMain)) end)
...
---------------------------------------------------------------------
-- V9 DETAIL PATCH: DIAGNOSTICS / SELF-HEALING / EMERGENCY CONTROLS
-- This section is intentionally verbose because the script is used in many
-- Delta/mobile environments where failures can be silent. The goal is to make
-- future debugging easier and make the GUI recover if a touch handler fails.
---------------------------------------------------------------------

local FE_Diagnostics = {
    Version = "V9_DETAIL_PATCH_1",
    Logs = {},
    MaxLogs = 80,
    LastHealthCheck = 0,
    LastVisibleState = true,
    EmergencyControlsCreated = false,
...
local function diagToString(value)
...
local function diagLog(message, good)
    local line = os.date("%H:%M:%S") .. " | " .. tostring(message)
    table.insert(FE_Diagnostics.Logs, 1, line)
    while #FE_Diagnostics.Logs > FE_Diagnostics.MaxLogs do
        table.remove(FE_Diagnostics.Logs)
    end
    if good == false then
        FE_Diagnostics.LastError = line
    end
    pcall(function()
        warn("[FE_BUNDLE] " .. line)
    end)
...
local function diagSafe(label, callback)
    local ok, result = pcall(callback)
    if not ok then
        diagLog(label .. " failed: " .. tostring(result), false)
        return false, result
    end
    diagLog(label .. " ok", true)
    return true, result
end

local function diagSetStatus(text, good)
    status("[diag] " .. tostring(text), good)
    diagLog(text, good)
end

local function diagGetMainState()
    return {
        screenGui = ScreenGui ~= nil and ScreenGui.Parent ~= nil,
        main = Main ~= nil and Main.Parent ~= nil,
        mainVisible = Main ~= nil and Main.Visible == true,
        icon = IconButton ~= nil and IconButton.Parent ~= nil,
        body = Body ~= nil and Body.Parent ~= nil,
        page = CurrentPage,
        bundleResults = #BundleResults,
        emoteResults = #EmoteResults,
        favoritesBundles = #FavoriteBundles,
        favoritesEmotes = #FavoriteEmotes,
        savedPacks = #SavedPacks,
        canSave = CAN_SAVE,
        lastError = FE_Diagnostics.LastError
    }
end

local function diagShowNotification(title, text)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = tostring(title or "FE_BUNDLE"),
...
local function diagMakeMiniButton(parent, text, pos, callback, color)
    local button = new("TextButton", {
        Parent = parent,
        Position = pos,
        Size = UDim2.new(0, 74, 0, 28),
        BackgroundColor3 = color or Theme.Cyan,
        BorderSizePixel = 0,
        Text = text,
        TextColor3 = Theme.Text,
        TextStrokeTransparency = 1,
    ...
    corner(button, 8)
    stroke(button, Theme.Black, 1, 0)
    add(Connections, button.MouseButton1Click:Connect(function()
        diagSafe("mini button " .. text, callback)
    end))
    pcall(function()
        add(Connections, button.Activated:Connect(function()
            diagSafe("mini button activated " .. text, callback)
        end))
    end)
    return button
end

local EmergencyFrame = nil
local LogFrame = nil

local function diagOpenMain(reason)
    if not Main or not Main.Parent then
        diagLog("diagOpenMain no Main, recreating GUI", false)
        diagSafe("createGui from diagOpenMain", function()
            createGui()
        end)
    end
    if Main then
        Main.Visible = true
        Main.Size = UDim2.new(0, 570, 0, 535)
        Main.BackgroundTransparency = 0
    end
    if Body and not Body:FindFirstChildWhichIsA("GuiObject") then
        diagSafe("renderHome from diagOpenMain", function()
            renderHome("Bundle")
        end)
    end
    diagSetStatus("opened main" .. (reason and (" | " .. reason) or ""), true)
end

local function diagCloseMain()
    if Main then
        tween(Main, {Size = UDim2.new(0, 40, 0, 40), BackgroundTransparency = 0.35}, 0.12)
    ...
    diagLog("closed main", true)
end

local function diagDumpLogsToStatus()
    local msg = FE_Diagnostics.Logs[1] or "No logs"
    status(msg, FE_Diagnostics.LastError == "None")
    diagShowNotification("FE_BUNDLE LOG", msg)
end

local function diagCreateLogWindow()
    ...
    corner(LogFrame, 12)
    stroke(LogFrame, Theme.Black, 2, 0)
    makeLabel(LogFrame, "FE_BUNDLE Diagnostics", UDim2.new(0, 12, 0, 8), UDim2.new(1, -48, 0, 24), 14, Theme.Text)
    local close = diagMakeMiniButton(LogFrame, "X", UDim2.new(1, -42, 0, 8), function()
        if LogFrame then LogFrame:Destroy(); LogFrame = nil end
    end, Theme.Red)
    ...
    local logScroll = new("ScrollingFrame", {
        ...
        CanvasSize = UDim2.new(0, 0, 0, math.max(220, #FE_Diagnostics.Logs * 24 + 20)),
        ZIndex = 5101
    })
    corner(logScroll, 8)
    stroke(logScroll, Theme.Black, 1, 0.3)

    local y = 8
    for i = 1, math.min(#FE_Diagnostics.Logs, 60) do
        makeLabel(logScroll, FE_Diagnostics.Logs[i], UDim2.new(0, 8, 0, y), UDim2.new(1, -16, 0, 22), 10, Theme.Muted)
        y = y + 24
    end
    logScroll.CanvasSize = UDim2.new(0, 0, 0, y + 10)
end

local function diagCreateEmergencyControls()
    if FE_Diagnostics.EmergencyControlsCreated then return end
    if not ScreenGui then return end
    FE_Diagnostics.EmergencyControlsCreated = true

    EmergencyFrame = new("Frame", {
        Parent = ScreenGui,
        Position = UDim2.new(0, 110, 0, 12),
        Size = UDim2.new(0, 250, 0, 34),
        BackgroundTransparency = 1,
        ZIndex = 5000
    })

    diagMakeMiniButton(EmergencyFrame, "SHOW", UDim2.new(0, 0, 0, 0), function()
        diagOpenMain("emergency show")
    end, Theme.Green)

    diagMakeMiniButton(EmergencyFrame, "HIDE", UDim2.new(0, 82, 0, 0), function()
        diagCloseMain()
    end, Theme.Yellow)

    diagMakeMiniButton(EmergencyFrame, "LOG", UDim2.new(0, 164, 0, 0), function()
        diagCreateLogWindow()
    end, Theme.Cyan)

    -- Global fallback: if user touches the SHOW button area and normal button events fail,
    -- still open the hub by coordinate check.
    add(Connections, UserInputService.InputBegan:Connect(function(input)
        if input.UserInputType ~= Enum.UserInputType.Touch and input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
        if not EmergencyFrame then return end
        local pos = input.Position
        local p = EmergencyFrame.AbsolutePosition
        local s = EmergencyFrame.AbsoluteSize
        if pos.X >= p.X and pos.X <= p.X + s.X and pos.Y >= p.Y and pos.Y <= p.Y + s.Y then
            if pos.X <= p.X + 78 then
                diagOpenMain("global emergency area")
            elseif pos.X <= p.X + 160 then
                diagCloseMain()
            else
                diagCreateLogWindow()
            end
    ...
    diagLog("emergency controls created", true)
end

local function diagRunPreflight()
    diagLog("preflight started", true)
    FE_Diagnostics.GuiParentOkay = ScreenGui ~= nil and ScreenGui.Parent ~= nil
    FE_Diagnostics.OpenButtonOkay = IconButton ~= nil and IconButton.Parent ~= nil

    diagSafe("http catalog ping", function()
        local raw = httpGet("https://catalog.roblox.com/v1/search/items/details?Category=12&Subcategory=39&Keyword=wave&Limit=10")
        FE_Diagnostics.HttpOkay = type(raw) == "string" and #raw > 10
        return FE_Diagnostics.HttpOkay
    end)

    diagSafe("get objects soft test", function()
        -- harmless test with zero/invalid id may error in some games; this is only a capability probe
        local ok = pcall(function()
            game:GetObjects("rbxassetid://0")
        end)
        FE_Diagnostics.GetObjectsOkay = ok
        return ok
    end)

    diagLog("preflight state: " .. diagToString(diagGetMainState()), true)
    FE_Diagnostics.SelfTestFinished = true
end

local function diagHealthLoop()
    task.spawn(function()
        while Alive do
            task.wait(5)
            if not Alive then break end
            FE_Diagnostics.LastHealthCheck = os.clock()
            if ScreenGui and not ScreenGui.Parent then
                diagLog("ScreenGui lost parent", false)
            end
            if Main and FE_Diagnostics.LastVisibleState ~= Main.Visible then
                FE_Diagnostics.LastVisibleState = Main.Visible
                diagLog("Main visibility changed: " .. tostring(Main.Visible), true)
            end
            if IconButton and IconButton.Parent then
...
-- Install the emergency layer and diagnostics after the main script has booted.
diagSafe("install emergency controls", function()
    diagCreateEmergencyControls()
end)

diagSafe("run preflight async", function()
    task.spawn(function()
        task.wait(1)
        diagRunPreflight()
    end)
...
diagLog("FE_BUNDLE V9 detail patch loaded", true)
