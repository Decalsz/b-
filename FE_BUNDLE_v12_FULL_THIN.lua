...
====================================================================
Version: 12.1.0 — thin systems filled
Schema: 1
...
local function playEmote(catalogOrAnimId, name)
    ...
    if not realId or realId == "" then
        ...
        status("No animation ID", false)
        notify("FE_BUNDLE", "Animation failed")
        logErr("EMOTE", "resolve failed for " .. tostring(catalogOrAnimId))
        return false
    end
    local animator = getAnimator()
    if not animator then
        App.EmoteState = "FAILED"
        status("No Animator", false)
        logErr("EMOTE", "no animator")
        return false
    end
    ...
    if App.CurrentEmoteTrack then
        ...
        pcall(function() App.CurrentEmoteTrack:Stop(0.08); App.CurrentEmoteTrack:Destroy() end)
        App.CurrentEmoteTrack = nil
    end
    stopReverse()
    ...
    if not ok or not track then
        ...
        notify("FE_BUNDLE", "Animation failed")
        logErr("EMOTE", "LoadAnimation failed")
        return false
    end
    ...
    pcall(function()
        if App.Settings.MoveWhileEmote then
            track.Priority = Enum.AnimationPriority.Action
        else
            track.Priority = Enum.AnimationPriority.Action4
        end
        track.Looped = App.Settings.EmoteLoop == true
        local spd = App.Settings.EmoteSpeed
        if spd == 0 then
            track:Play(0.12, 1, 0)
            App.EmoteState = "PAUSED"
        else
            track:Play(0.12, 1, spd)
        end
    end)

    -- ended handler for non-loop
    ...
    -- Crowdsource (optional, privacy-respecting: no username)
    if App.Settings.Crowdsource then
        task.spawn(function()
            pcall(function()
                local placeId = game.PlaceId
                local anim = tostring(realId)
                -- fire-and-forget; ignore failures
                httpGet("https://httpbin.org/get?fe_bundle_crowd=1&place="..tostring(placeId).."&anim="..anim)
            end)
        end)
    end
    updateEmoteRuntimeUI()
...
-- Toggle: same emote while playing = STOP; different = switch
