...
---------------------------------------------------------------------
-- HARD BOOT GUARD (visible even if later code errors)
---------------------------------------------------------------------
do
    local function hardParent()
        local pg
        pcall(function() if gethui then pg = gethui() end end)
        if not pg then pcall(function() pg = game:GetService("CoreGui") end) end
        if not pg then
            pcall(function()
                pg = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui", 5)
            end)
        end
        return pg
    end
    local parent = hardParent()
    if parent then
        pcall(function()
            local old = parent:FindFirstChild("FE_BUNDLE_HARD_BOOT")
            if old then old:Destroy() end
        end)
        local sg = Instance.new("ScreenGui")
        sg.Name = "FE_BUNDLE_HARD_BOOT"
        ...
        corner.Parent = btn
        _G.__FE_BUNDLE_BOOT_BTN = btn
        _G.__FE_BUNDLE_BOOT_SG = sg
        pcall(function() warn("[FE_BUNDLE] HARD BOOT icon created") end)
    else
        pcall(function() warn("[FE_BUNDLE] HARD BOOT failed: no parent gui") end)
    end
end

function __feBootStatus(text, ok)
    pcall(function()
        warn("[FE_BUNDLE] " .. tostring(text))
        local btn = _G.__FE_BUNDLE_BOOT_BTN
        if btn and btn.Parent then
            btn.Text = tostring(text):sub(1, 18)
            btn.BackgroundColor3 = ok and Color3.fromRGB(40, 180, 90) or Color3.fromRGB(220, 60, 60)
        end
    end)
end

function __feBootError(err)
    __feBootStatus("ERR", false)
    pcall(function()
        warn("[FE_BUNDLE] FATAL: " .. tostring(err))
        local sg = _G.__FE_BUNDLE_BOOT_SG
        if not sg then return end
        local f = Instance.new("TextLabel")
        f.Size = UDim2.new(0, 320, 0, 120)
        f.Position = UDim2.new(0, 12, 0, 170)
        f.BackgroundColor3 = Color3.fromRGB(30, 10, 10)
        f.TextColor3 = Color3.fromRGB(255, 180, 180)
...
function add(bucket, conn)
    table.insert(bucket or Conns.Global, conn)
    return conn
end

function disconnect(bucket)
...
function label(parent, text, pos, size, textSize, color)
...
function button(parent, text, pos, size, callback, color, bucket)
...
function textBox(parent, placeholder, pos, size, initial)
...
function panel(parent, pos, size, color)
...
function scrollFrame(parent, pos, size)
...
function buildCatalogUrl(kind, query, cursor)
...
function searchCatalog(kind, query, append)
...
function categorizeAnimation(pathText)
...
function scanAnimationTree(root, path, output)
...
function resolveBundleAsset(assetId)
    assetId = normalizeId(assetId)
...
function extractAnimationsFromBundle(details)
    local form = {}
...
function resolveEmoteAnimation(assetId)
    assetId = normalizeId(assetId)
    if assetId == "" then return nil end
...
function getAnimationsForState(state)
...
function setStateAnimation(state, id)
    id = normalizeId(id)
...
function applyDescriptionAnimations()
    refreshCharacterReferences()
...
function applyCurrentForm(name)
    local changed = 0
    local descChanged = 0
...
function playEmote(itemOrId, name)
    local id, emoteName
...
function applyBundleFull(bundleId, bundleName)
...
function setCustomSlotFromBundle(state, bundleId, bundleName)
...
function applyControllerToTrack(track)
...
function reflowFloatingButtons()
...
function rebuildFloatingButtons()
...
function createFloatingButton(item)
    local id = tostring(item.id or item.Id or "")
    if id == "" then return end
...
function createQuickEntry(item)
...
function createPickerShortcut(item)
...
function createAvatarPreview(parent, animationId)
...
function showInfoModal(titleText, bodyText, imageId, actions, previewAnimationId)
    closeModal()
...
function favList(kind) return kind=="Emote" and State.Favorites.Emotes or State.Favorites.Bundles end
function isFavorite(kind,id) for _,f in ipairs(favList(kind)) do if tostring(f.id)==tostring(id) then return true end end return false end
...
function renderItemCard(parent,item,index,kind)
...
do
    local ok, err = pcall(function()
        __feBootStatus("LOAD DATA", true)
        loadData()
        __feBootStatus("CREATE GUI", true)
        createGui()
        __feBootStatus("LIFECYCLE", true)
        setupCharacterLifecycle()
        __feBootStatus("STARTUP", true)
        finalStartupInstall()
        __feBootStatus("FE READY", true)
        -- hide hard boot after real icon exists
        task.delay(1.5, function()
            pcall(function()
                if Gui and Gui.Screen and Gui.Icon and _G.__FE_BUNDLE_BOOT_SG then
                    _G.__FE_BUNDLE_BOOT_SG:Destroy()
                    _G.__FE_BUNDLE_BOOT_SG = nil
                    _G.__FE_BUNDLE_BOOT_BTN = nil
                end
            end)
        end)
    end)
...
---------------------------------------------------------------------
-- STAGE 12: RESPAWN STICKY APPLY (minimal patch — do not rewrite other systems)
-- Bug: after death, Animate re-inits and overwrites bundle animations.
-- Fix: multi-retry strict reapply over several seconds + Animate ChildAdded watch.
-- Floating bundle support already exists; this stage does not remove it.
---------------------------------------------------------------------

local Stage12 = {
    Version = "12-respawn-sticky",
    ApplyToken = 0,
    WatchConn = nil,
}

if State.Settings.Stage12StickyRespawn == nil then State.Settings.Stage12StickyRespawn = true end
if State.Settings.Stage12StickySeconds == nil then State.Settings.Stage12StickySeconds = 8 end

function stage12HasForm()
    for _, s in ipairs(STATES) do
        if normalizeId(State.CurrentForm[s]) ~= "" then return true end
    end
    if type(State.CurrentMappings) == "table" and #State.CurrentMappings > 0 then return true end
    return false
end

function stage12RecoverMappingsSync()
    if type(State.CurrentMappings) == "table" and #State.CurrentMappings > 0 then
        return true
    end
    local bundleId = nil
    local bundleName = State.LastAppliedName
    if type(State.SlotMeta) == "table" then
        for _, meta in pairs(State.SlotMeta) do
            if type(meta) == "table" and normalizeId(meta.BundleId) ~= "" then
                bundleId = normalizeId(meta.BundleId)
                bundleName = meta.Bundle or bundleName
                break
            end
        end
    end
    if not bundleId or not stage9ResolveBundleMappings then return false end
    local ok, mappings = pcall(function()
        return stage9ResolveBundleMappings(bundleId)
    end)
    if ok and type(mappings) == "table" and #mappings > 0 then
        State.CurrentMappings = mappings
        if not State.LastAppliedName or State.LastAppliedName == "" then
            State.LastAppliedName = bundleName or ("Bundle " .. bundleId)
        end
        if stage91SaveSnapshot then pcall(stage91SaveSnapshot) end
        log("Stage12 recovered mappings " .. tostring(bundleId))
        return true
    end
    return false
end

function stage12DoApply(reason)
    if not stage12HasForm() then return false end
    refreshCharacterReferences()
    if not State.Character.Humanoid or not State.Character.Animate then return false end
    pcall(stage12RecoverMappingsSync)
    local applied = false
    if State.Settings.StrictBundleReplace ~= false and stage11StrictApplyCurrent then
        local ok = stage11StrictApplyCurrent(reason or "Stage12")
        applied = ok == true
    end
    if not applied and applyCurrentForm then
        local ok2 = pcall(function()
            applyCurrentForm(State.LastAppliedName or reason or "Stage12")
        end)
        applied = ok2
    end
    return applied
...
function stage12StickyReapply(reason)
    if State.Settings.Stage12StickyRespawn == false then return end
    if not stage12HasForm() then
        setStatus("No saved form to reapply", false)
        return
    end
    Stage12.ApplyToken = Stage12.ApplyToken + 1
    local token = Stage12.ApplyToken
    local delays = {0.2, 0.5, 0.9, 1.4, 2.0, 2.8, 3.8, 5.0, 6.5, 8.0}
    local maxSec = tonumber(State.Settings.Stage12StickySeconds) or 8
    log("Stage12 sticky: " .. tostring(reason))
    task.spawn(function()
        local last = 0
        local successCount = 0
        for _, d in ipairs(delays) do
            if token ~= Stage12.ApplyToken then return end
            if d > maxSec then break end
            task.wait(math.max(0.01, d - last))
            last = d
            if token ~= Stage12.ApplyToken then return end
            if not LocalPlayer.Character then return end
            refreshCharacterReferences()
            if State.Character.Humanoid and State.Character.Animate then
                if stage12DoApply(tostring(reason) .. " @" .. tostring(d) .. "s") then
                    successCount = successCount + 1
                    setStatus("Sticky OK x" .. tostring(successCount) .. " @" .. tostring(d) .. "s", true)
                end
            end
        end
        if successCount > 0 then
            notify("Pack stuck after respawn (" .. tostring(successCount) .. "x)", true)
        else
            setStatus("Sticky reapply failed (no Animate/form)", false)
        end
    end)
end

function stage12WatchAnimate()
    if Stage12.WatchConn then
        pcall(function() Stage12.WatchConn:Disconnect() end)
        Stage12.WatchConn = nil
    end
    refreshCharacterReferences()
    local animate = State.Character.Animate
    if not animate then return end
    local debounce = false
    Stage12.WatchConn = animate.ChildAdded:Connect(function()
        if State.Settings.Stage12StickyRespawn == false then return end
        if not stage12HasForm() then return end
        if debounce then return end
        debounce = true
        task.delay(0.45, function()
            debounce = false
            if stage12HasForm() then
                refreshCharacterReferences()
                stage12DoApply("Animate watch")
            end
        end)
    end)
    add(Connections.Character, Stage12.WatchConn)
end

local stage12PrevLifecycle = setupCharacterLifecycle
setupCharacterLifecycle = function()
    if stage12PrevLifecycle then pcall(stage12PrevLifecycle) end
    disconnect(Connections.Character)
    add(Connections.Character, LocalPlayer.CharacterRemoving:Connect(function()
        stopEmote("Character removing")
        if stopControllerReverse then pcall(stopControllerReverse) end
        if Stage12.WatchConn then
            pcall(function() Stage12.WatchConn:Disconnect() end)
            Stage12.WatchConn = nil
        end
    ...
    add(Connections.Character, LocalPlayer.CharacterAdded:Connect(function()
        ...
        if stopControllerReverse then pcall(stopControllerReverse) end
        task.spawn(function()
            task.wait(tonumber(State.Settings.AutoReapplyDelay) or 0.15)
            refreshCharacterReferences()
            if captureOriginalAnimations then pcall(captureOriginalAnimations) end
            stage12WatchAnimate()
            if State.Settings.AutoReapplyOnRespawn ~= false then
                stage12StickyReapply("respawn")
            end
            if rebuildFloatingButtons then pcall(rebuildFloatingButtons) end
            if rebuildQuickSelector then pcall(rebuildQuickSelector) end
            if updateRuntimeBar then pcall(updateRuntimeBar) end
        end)
    end))
    refreshCharacterReferences()
    if captureOriginalAnimations then pcall(captureOriginalAnimations) end
...
local stage12PrevRenderSettings = renderSettings
renderSettings = function()
    stage12PrevRenderSettings()
    local sc = nil
    for _, c in ipairs(Gui.Body:GetChildren()) do
        if c:IsA("ScrollingFrame") then sc = c break end
    end
    if not sc then return end
    local y = sc.CanvasSize.Y.Offset + 8
    label(sc, "Respawn Sticky Fix (Stage 12)", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,24), 16, THEME.Text)
    y = y + 34
    button(sc, State.Settings.Stage12StickyRespawn and "Sticky Respawn: ON" or "Sticky Respawn: OFF", UDim2.new(0,12,0,y), UDim2.new(0,180,0,32), function()
        State.Settings.Stage12StickyRespawn = not State.Settings.Stage12StickyRespawn
        saveData()
        renderSettings()
    end, State.Settings.Stage12StickyRespawn and THEME.Green or THEME.Card)
    button(sc, "REAPPLY NOW", UDim2.new(0,204,0,y), UDim2.new(0,120,0,32), function()
        stage12StickyReapply("manual")
    end, THEME.Yellow)
    y = y + 42
    label(sc, "Multi-retry strict reapply after death (~8s) so Animate cannot wipe your bundle.", UDim2.new(0,12,0,y), UDim2.new(1,-24,0,40), 12, THEME.Muted)
    sc.CanvasSize = UDim2.new(0,0,0,y + 60)
end

task.defer(function()
    pcall(function() setupCharacterLifecycle() end)
    if State.Settings.AutoApplyOnExecute ~= false and stage12HasForm() then
        task.delay(0.7, function() stage12StickyReapply("execute") end)
    end
    notify("Stage 12 sticky respawn loaded", true)
    log("Stage 12 ready")
end)
